...
typedef struct _lv_draw_ctx_t {
   ...
   * @param dest_area     get this area of the result image. It assumes that the
   * original image is placed to the 0;0 position.
   ...
   * @param draw_dsc      an `lv_draw_img_dsc_t` descriptor containing the
   * transformation parameters
   * @param cf            the color format of `src_buf`
   * @param cbuf          place the colors of the pixels on `dest_area` here in
   * RGB format
   * @param abuf          place the opacity of the pixels on `dest_area` here
   */
  void (*draw_transform)(struct _lv_draw_ctx_t *draw_ctx,
                         const lv_area_t *dest_area, const void *src_buf,
   ...
   * @note dest_area and src_area must be clipped to the real dimensions of the
   * buffers
   */
  void (*buffer_copy)(struct _lv_draw_ctx_t *draw_ctx, void *dest_buf,
                      lv_coord_t dest_stride, const lv_area_t *dest_area,
                      void *src_buf, lv_coord_t src_stride,
                      const lv_area_t *src_area);

  /**
   * Initialize a new layer context.
   * The original buffer and area data are already saved from `draw_ctx` to
   * `layer_ctx`
  ...
  struct _lv_draw_layer_ctx_t *(*layer_init)(
      struct _lv_draw_ctx_t *draw_ctx, struct _lv_draw_layer_ctx_t *layer_ctx,
   ...
   * @param flags         OR-ed flags from @lv_draw_layer_flags_t
   */
  void (*layer_adjust)(struct _lv_draw_ctx_t *draw_ctx,
   ...
   */
  void (*layer_blend)(struct _lv_draw_ctx_t *draw_ctx,
                      struct _lv_draw_layer_ctx_t *layer_ctx,
                      const lv_draw_img_dsc_t *draw_dsc);

  /**
   * Destroy a layer context. The original buffer and area data of the
   * `draw_ctx` will be restored and the `layer_ctx` itself will be freed
   * automatically.
   * @param draw_ctx      pointer to the current draw context
   * @param layer_ctx     pointer to a layer context
   */
  void (*layer_destroy)(struct _lv_draw_ctx_t *draw_ctx,
