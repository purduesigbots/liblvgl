...
/**
 * Call it in the super-loop of main() or threads. It will run
 * lv_timer_handler() with a given period in ms. You can use it with sleep or
 * delay in OS environment. This function is used to simplify the porting.
 * @param __ms the period for running lv_timer_handler()
 */
static inline LV_ATTRIBUTE_TIMER_HANDLER uint32_t
lv_timer_handler_run_in_period(uint32_t ms) {
