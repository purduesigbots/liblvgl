...
#define LV_STYLE_PROP_EXT_DRAW                                                 \
  (1 << 1) /*Requires ext. draw size update when changed*/
#define LV_STYLE_PROP_LAYOUT_REFR                                              \
  (1 << 2) /*Requires layout update when changed*/
#define LV_STYLE_PROP_PARENT_LAYOUT_REFR                                       \
  (1 << 3) /*Requires layout update on parent when changed*/
...
#define LV_STYLE_PROP_META_MASK                                                \
  (LV_STYLE_PROP_META_INHERIT | LV_STYLE_PROP_META_INITIAL)

#define LV_STYLE_PROP_ID_MASK(prop)                                            \
  ((lv_style_prop_t)((prop) & ~LV_STYLE_PROP_META_MASK))
 ...
 *
 * Props are split into groups of 16. When adding a new prop to a group, ensure
 * it does not overflow into the next one.
 */
typedef enum {
  LV_STYLE_PROP_INV = 0,
...
enum { LV_STYLE_RES_NOT_FOUND, LV_STYLE_RES_FOUND, LV_STYLE_RES_INHERIT };
 ...
 * @return a new property ID, or LV_STYLE_PROP_INV if there are no more
 * available.
 * @example
 ...
 */
static inline lv_style_res_t
lv_style_get_prop_inlined(const lv_style_t *style, lv_style_prop_t prop,
                          lv_style_value_t *value) {
  if (style->prop1 == LV_STYLE_PROP_ANY) {
    ...
    for (i = 0; i < style->prop_cnt; i++) {
      ...
      if (prop_id == prop) {
        ...
        *value = (const_prop->prop & LV_STYLE_PROP_META_INITIAL)
  ...
  if (style->prop_cnt > 1) {
    ...
    if (style->prop1 & LV_STYLE_PROP_META_INHERIT)
      return LV_STYLE_RES_INHERIT;
    *value =
        (style->prop1 & LV_STYLE_PROP_META_INITIAL)
            ? lv_style_prop_get_default(LV_STYLE_PROP_ID_MASK(style->prop1))
            : style->v_p.value1;
    return LV_STYLE_RES_FOUND;
  }
  return LV_STYLE_RES_NOT_FOUND;
