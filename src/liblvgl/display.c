...
static void lvgl_display_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area,
                               lv_color_t *color) {
  if (screen_copy_area(area->x1, area->y1, area->x2, area->y2,
                       (uint32_t *)color, area->x2 - area->x1 + 1) != 1) {
    errno = PROS_ERR;
  } else {
...
void display_initialize(void) {
  ...
  disp_daemon_task =
      task_create(disp_daemon, NULL, TASK_PRIORITY_MIN + 2,
