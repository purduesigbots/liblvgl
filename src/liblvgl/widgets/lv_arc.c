...
static void get_center(const lv_obj_t *obj, lv_point_t *center,
                       lv_coord_t *arc_r);
...
void lv_arc_align_obj_to_angle(const lv_obj_t *obj, lv_obj_t *obj_to_align,
                               lv_coord_t r_offset) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
void lv_arc_rotate_obj_to_angle(const lv_obj_t *obj, lv_obj_t *obj_to_rotate,
                                lv_coord_t r_offset) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
static void value_update(lv_obj_t *obj) {
  ...
  case LV_ARC_MODE_SYMMETRICAL:
    ...
    break;
  case LV_ARC_MODE_REVERSE:
    angle = lv_map(arc->value, arc->min_value, arc->max_value, bg_end,
                   arc->bg_angle_start);
