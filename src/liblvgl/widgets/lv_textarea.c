...
void lv_textarea_set_password_bullet(lv_obj_t *obj, const char *bullet) {
  ...
  if (!bullet && (ta->pwd_bullet)) {
    ...
    if (ta->pwd_bullet == NULL) {
      LV_LOG_ERROR("lv_textarea_set_password_bullet: couldn't allocate memory "
                   "for bullet");
      return;
    }
...
/**
 * Test a unicode character if it is accepted or not. Checks max length and
 * accepted char list.
...
static void refr_cursor_area(lv_obj_t *obj) {
  ...
  lv_coord_t letter_w =
      lv_font_get_glyph_width(font, letter_space, IGNORE_KERNING);
  ...
  if (((letter_pos.x + ta->label->coords.x1) + letter_w >
       ta->label->coords.x2) &&
