...
void lv_img_set_src(lv_obj_t *obj, const void *src) {
  ...
  if (src_type == LV_IMG_SRC_SYMBOL) {
    /*`lv_img_dsc_get_info` couldn't set the width and height of a font so set
     * it here*/
...
void lv_img_set_angle(lv_obj_t *obj, int16_t angle) {
  ...
  lv_area_t a;
  _lv_img_buf_get_transformed_area(&a, w, h, img->angle, img->zoom,
                                   &img->pivot);
  a.x1 += obj->coords.x1;
  a.y1 += obj->coords.y1;
  a.x2 += obj->coords.x1;
...
void lv_img_set_zoom(lv_obj_t *obj, uint16_t zoom) {
  ...
  lv_coord_t h = lv_obj_get_height(obj);
  lv_area_t a;
  _lv_img_buf_get_transformed_area(&a, w, h, img->angle, img->zoom >> 8,
                                   &img->pivot);
...
static lv_point_t lv_img_get_transformed_size(lv_obj_t *obj) {
  ...
  _lv_img_buf_get_transformed_area(&area_transform, img->w, img->h, img->angle,
                                   img->zoom, &img->pivot);
...
static void lv_img_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_STYLE_CHANGED) {
    ...
    if (img->w == lv_obj_get_width(obj) && img->h == lv_obj_get_height(obj) &&
        (img->zoom != LV_IMG_ZOOM_NONE || img->angle != 0 ||
         img->pivot.x != img->w / 2 || img->pivot.y != img->h / 2)) {

      lv_coord_t w = lv_obj_get_width(obj);
      lv_coord_t h = lv_obj_get_height(obj);
      lv_area_t coords;
      _lv_img_buf_get_transformed_area(&coords, w, h, img->angle, img->zoom,
                                       &img->pivot);
  ...
  if (code == LV_EVENT_COVER_CHECK) {
    ...
    if (img->zoom == LV_IMG_ZOOM_NONE) {
      ...
      _lv_img_buf_get_transformed_area(&a, lv_obj_get_width(obj),
                                       lv_obj_get_height(obj), 0, img->zoom,
                                       &img->pivot);
      a.x1 += obj->coords.x1;
      a.y1 += obj->coords.y1;
      a.x2 += obj->coords.x1;
    ...
    if (img->obj_size_mode == LV_IMG_SIZE_MODE_REAL) {
      ...
      _lv_img_buf_get_transformed_area(&bg_coords, obj_w, obj_h, img->angle,
                                       img->zoom, &bg_pivot);
