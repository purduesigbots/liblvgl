...
static void get_cell_area(lv_obj_t *obj, uint16_t row, uint16_t col,
                          lv_area_t *area);

static inline bool is_cell_empty(void *cell) { return cell == NULL; }
...
void lv_table_set_col_cnt(lv_obj_t *obj, uint16_t col_cnt) {
  ...
  table->col_w =
      lv_mem_realloc(table->col_w, col_cnt * sizeof(table->col_w[0]));
...
bool lv_table_has_cell_ctrl(lv_obj_t *obj, uint16_t row, uint16_t col,
  ...
  if (is_cell_empty(table->cell_data[cell]))
    ...
    return (table->cell_data[cell][0] & ctrl) == ctrl;
}
  ...
  for (row = 0; row < table->row_cnt; row++) {
    ...
    for (col = 0; col < table->col_cnt; col++) {
      ...
      for (col_merge = 0; col_merge + col < table->col_cnt - 1; col_merge++) {
        ...
        lv_table_cell_ctrl_t merge_ctrl =
            (lv_table_cell_ctrl_t)next_cell_data[0];
      ...
      if (table->cell_data[cell]) {
        const lv_coord_t cell_left =
            lv_obj_get_style_pad_left(obj, LV_PART_ITEMS);
        const lv_coord_t cell_right =
            lv_obj_get_style_pad_right(obj, LV_PART_ITEMS);
        const lv_coord_t cell_top =
            lv_obj_get_style_pad_top(obj, LV_PART_ITEMS);
        const lv_coord_t cell_bottom =
...
static void refr_size_form_row(lv_obj_t *obj, uint32_t start_row) {
  const lv_coord_t cell_pad_left =
      lv_obj_get_style_pad_left(obj, LV_PART_ITEMS);
  const lv_coord_t cell_pad_right =
      lv_obj_get_style_pad_right(obj, LV_PART_ITEMS);
  const lv_coord_t cell_pad_top = lv_obj_get_style_pad_top(obj, LV_PART_ITEMS);
  const lv_coord_t cell_pad_bottom =
      lv_obj_get_style_pad_bottom(obj, LV_PART_ITEMS);
  ...
  for (i = start_row; i < table->row_cnt; i++) {
    lv_coord_t calculated_height =
        get_row_height(obj, i, font, letter_space, line_space, cell_pad_left,
...
static void refr_cell_size(lv_obj_t *obj, uint32_t row, uint32_t col) {
  ...
  lv_table_t *table = (lv_table_t *)obj;
  lv_coord_t calculated_height =
      get_row_height(obj, row, font, letter_space, line_space, cell_pad_left,
                     cell_pad_right, cell_pad_top, cell_pad_bottom);
...
static lv_coord_t get_row_height(lv_obj_t *obj, uint16_t row_id,
  ...
  for (cell = row_start, col = 0; cell < row_start + table->col_cnt;
     ...
     * Increment the text width if the cell has the
     * LV_TABLE_CELL_CTRL_MERGE_RIGHT control, exit the traversal when the
     * current cell control is not LV_TABLE_CELL_CTRL_MERGE_RIGHT */
    uint16_t col_merge = 0;
    for (col_merge = 0; col_merge + col < table->col_cnt - 1; col_merge++) {
      char *next_cell_data = table->cell_data[cell + col_merge];
    ...
    /*When cropping the text we can assume the row height is equal to the line
     * height*/
    if (ctrl & LV_TABLE_CELL_CTRL_TEXT_CROP) {
      h_max =
          LV_MAX(lv_font_get_line_height(font) + cell_top + cell_bottom, h_max);
    }
    /*Else we have to calculate the height of the cell text*/
      ...
      lv_txt_get_size(&txt_size, table->cell_data[cell] + 1, font, letter_space,
