...
LV_ATTRIBUTE_FAST_MEM static void
fill_argb(lv_color_t *dest_buf, const lv_area_t *dest_area,
          lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa,
          const lv_opa_t *mask, lv_coord_t mask_stride);
...
#if LV_COLOR_SCREEN_TRANSP
LV_ATTRIBUTE_FAST_MEM static void
map_argb(lv_color_t *dest_buf, const lv_area_t *dest_area,
         lv_coord_t dest_stride, const lv_color_t *src_buf,
         lv_coord_t src_stride, lv_opa_t opa, const lv_opa_t *mask,
...
lv_draw_sw_blend_basic(lv_draw_ctx_t *draw_ctx,
  ...
  if (disp->driver->set_px_cb == NULL) {
    if (disp->driver->screen_transp == 0) {
      ...
      dest_buf8 += dest_stride * (blend_area.y1 - draw_ctx->buf_area->y1) *
                   LV_IMG_PX_SIZE_ALPHA_BYTE;
      dest_buf8 +=
          (blend_area.x1 - draw_ctx->buf_area->x1) * LV_IMG_PX_SIZE_ALPHA_BYTE;
  ...
  if (disp->driver->set_px_cb) {
    ...
    if (dsc->src_buf == NULL) {
      fill_argb(dest_buf, &blend_area, dest_stride, dsc->color, dsc->opa, mask,
                mask_stride);
    } else {
      map_argb(dest_buf, &blend_area, dest_stride, src_buf, src_stride,
...
fill_normal(lv_color_t *dest_buf, const lv_area_t *dest_area,
  ...
  if (mask == NULL) {
    ...
    /*Has opacity*/
       ...
       *Introduce the same error here too to make lv_color_premult produces the
       *same result */
...
static inline void
set_px_argb_blend(uint8_t *buf, lv_color_t color, lv_opa_t opa,
  ...
  if (last_dest_color.full != bg_color.full ||
      last_src_color.full != color.full || last_opa != opa) {
...
fill_argb(lv_color_t *dest_buf, const lv_area_t *dest_area,
  ...
  if (mask == NULL) {
    if (opa >= LV_OPA_MAX) {
      ...
      for (y = 1; y < h; y++) {
        lv_memcpy(dest_buf8, (uint8_t *)dest_buf,
...
static void map_blended(lv_color_t *dest_buf, const lv_area_t *dest_area,
  ...
  if (mask == NULL) {
    ...
    for (y = 0; y < h; y++) {
      for (x = 0; x < w; x++) {
        if (last_src_color.full != src_buf[x].full ||
            last_dest_color.full != dest_buf[x].full) {
          last_dest_color = dest_buf[x];
  ...
  /*Masked*/
    ...
    lv_color_t last_res_color =
        blend_fp(last_src_color, last_dest_color, last_opa);
    for (y = 0; y < h; y++) {
      for (x = 0; x < w; x++) {
        if (mask[x] == 0)
          continue;
        ...
        if (last_src_color.full != src_buf[x].full ||
            last_dest_color.full != dest_buf[x].full || last_opa != opa_tmp) {
          last_dest_color = dest_buf[x];
          last_src_color = src_buf[x];
          last_opa = opa_tmp;
          last_res_color = blend_fp(last_src_color, last_dest_color, last_opa);
