...
void lv_draw_sw_letter(lv_draw_ctx_t *draw_ctx, const lv_draw_label_dsc_t *dsc,
  ...
  if (g_ret == false) {
    ...
    if (letter >= 0x20 && letter != 0xf8ff && /*LV_SYMBOL_DUMMY*/
      ...
      LV_LOG_WARN("lv_draw_letter: glyph dsc. not found for U+%" PRIX32,
                  letter);
      ...
      lv_area_set(&glyph_coords, begin_x, begin_y, begin_x + g.box_w,
                  begin_y + g.box_h);
  ...
  for (row = row_start; row < row_end; row++) {
    ...
    for (col = col_start; col < col_end; col++) {
      ...
      if (subpx_cnt == 3) {
        ...
        res_color.ch.red = (uint32_t)((uint16_t)txt_rgb[0] * font_rgb[2] +
                                      (bg_rgb[0] * (255 - font_rgb[2]))) >>
                           8;
        res_color.ch.blue = (uint32_t)((uint16_t)txt_rgb[2] * font_rgb[0] +
                                       (bg_rgb[2] * (255 - font_rgb[0]))) >>
                            8;
#else
