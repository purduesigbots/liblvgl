 ...
 *********************/
    ...
    LV_COLOR_DEPTH == 32 && !defined(__ARM_2D_LVGL_CFG_NO_WARNING__)
...
#define __PREPARE_LL_ACCELERATION__()                                          \
  ...
  if (!_lv_area_intersect(&blend_area2, &draw_area, draw_ctx->clip_area))      \
    return;                                                                    \
                                                                               \
...
#define __RECOLOUR_WRAPPER(...)                                                \
  do {                                                                         \
    ...
    if (draw_dsc->recolor_opa > LV_OPA_MIN) {                                  \
      rgb_tmp_buf = lv_mem_buf_get(src_w * src_h * sizeof(lv_color_t));        \
      if (NULL == rgb_tmp_buf) {                                               \
        LV_LOG_WARN("Failed to allocate memory for accelerating recolour, "    \
      ...
      __arm_2d_impl_colour_filling_with_opacity(                               \
          (color_int *)rgb_tmp_buf, src_w, &copy_size,                         \
          (color_int)draw_dsc->recolor.full, draw_dsc->recolor_opa);           \
                                                                               \
...
static bool lv_draw_arm2d_tile_copy(const arm_2d_tile_t *target_tile,
                                    const arm_2d_region_t *region,
                                    arm_2d_tile_t *source_tile, lv_opa_t opa,
                                    arm_2d_tile_t *mask_tile);
#else

static void convert_cb(const lv_area_t *dest_area, const void *src_buf,
                       lv_coord_t src_w, lv_coord_t src_h,
                       lv_coord_t src_stride, const lv_draw_img_dsc_t *draw_dsc,
                       lv_img_cf_t cf, lv_color_t *cbuf, lv_opa_t *abuf);

LV_ATTRIBUTE_FAST_MEM
static bool arm_2d_fill_normal(lv_color_t *dest_buf, const lv_area_t *dest_area,
                               lv_coord_t dest_stride, lv_color_t color,
                               lv_opa_t opa, const lv_opa_t *mask,
                               lv_coord_t mask_stride);

LV_ATTRIBUTE_FAST_MEM
static bool arm_2d_copy_normal(lv_color_t *dest_buf, const lv_area_t *dest_area,
...
LV_ATTRIBUTE_FAST_MEM
static void lv_draw_arm2d_blend(lv_draw_ctx_t *draw_ctx,
...
static void lv_draw_arm2d_blend(lv_draw_ctx_t *draw_ctx,
  ...
  if (dsc->blend_mode == LV_BLEND_MODE_NORMAL &&
    ...
    if (src_buf) {
      is_accelerated =
          lv_draw_arm2d_tile_copy(&target_tile, &target_region, &source_tile,
                                  dsc->opa, (NULL == mask) ? NULL : &mask_tile);
    } else {
      is_accelerated = lv_draw_arm2d_fill_colour(
          &target_tile, &target_region, dsc->color, dsc->opa,
...
static bool lv_draw_arm2d_fill_colour(const arm_2d_tile_t *target_tile,
  ...
  if (NULL == mask_tile) {
    if (opa >= LV_OPA_MAX) {
      ...
      return false;
#else
      result = arm_2d_fill_colour_with_alpha(target_tile, region,
                                             (arm_2d_color_t){color.full}, opa);
#endif
    }
  } else {

    if (opa >= LV_OPA_MAX) {
      result = arm_2d_fill_colour_with_mask(target_tile, region, mask_tile,
      ...
      result = arm_2d_fill_colour_with_mask_and_opacity(
          target_tile, region, mask_tile, (arm_2d_color_t){color.full}, opa);
#endif
    }
  }
...
static bool lv_draw_arm2d_tile_copy(const arm_2d_tile_t *target_tile,
  ...
  if (NULL == mask_tile) {
    if (opa >= LV_OPA_MAX) {
      result = arm_2d_tile_copy(source_tile, target_tile, region,
                                ARM_2D_CP_MODE_COPY);
    }
#if LV_COLOR_SCREEN_TRANSP
    else {
      ...
      result = arm_2d_alpha_blending(source_tile, target_tile, region, opa);
    }
#endif
  } else {
#if LV_COLOR_SCREEN_TRANSP
    ...
    if (opa >= LV_OPA_MAX) {
      result = arm_2d_tile_copy_with_src_mask(
          source_tile, mask_tile, target_tile, region, ARM_2D_CP_MODE_COPY);
    } else {
      return false;
    }
#endif
...
static void lv_draw_arm2d_blend(lv_draw_ctx_t *draw_ctx,
  ...
  do {
    ...
    if (mask) {
      ...
      mask += mask_stride * (blend_area.y1 - dsc->mask_area->y1) +
    ...
    if (dsc->src_buf == NULL) {
      if (dsc->blend_mode == LV_BLEND_MODE_NORMAL) {
        is_accelerated =
            arm_2d_fill_normal(dest_buf, &blend_area, dest_stride, dsc->color,
      ...
      if (dsc->blend_mode == LV_BLEND_MODE_NORMAL) {
        is_accelerated =
            arm_2d_copy_normal(dest_buf, &blend_area, dest_stride, src_buf,
                               src_stride, dsc->opa, mask, mask_stride);
      }
#if LV_DRAW_COMPLEX
      else {
        break;
...
static bool arm_2d_fill_normal(lv_color_t *dest_buf, const lv_area_t *dest_area,
  ...
  if (mask == NULL) {
    if (opa >= LV_OPA_MAX) {
      __arm_2d_impl_colour_filling((color_int *)dest_buf, dest_stride,
                                   &target_size, color.full);
    }
    /*Has opacity*/
    else {
#if LV_COLOR_SCREEN_TRANSP
...
#else
      __arm_2d_impl_colour_filling_with_opacity(
          (color_int *)dest_buf, dest_stride, &target_size, color.full, opa);
#endif
    }
  }
  /*Masked*/
    ...
    if (opa >= LV_OPA_MAX) {
      __arm_2d_impl_colour_filling_mask((color_int *)dest_buf, dest_stride,
                                        (uint8_t *)mask, mask_stride,
                                        &target_size, color.full);
    }
    /*With opacity*/
    else {
      ...
      __arm_2d_impl_colour_filling_mask_opacity(
          (color_int *)dest_buf, dest_stride, (uint8_t *)mask, mask_stride,
          &target_size, color.full, opa);
#endif
    }
  }
...

{
  ...
  if (mask == NULL) {
    if (opa >= LV_OPA_MAX) {
      __arm_2d_impl_copy((color_int *)src_buf, src_stride,
                         (color_int *)dest_buf, dest_stride, &copy_size);
    } else {
#if LV_COLOR_SCREEN_TRANSP
      return false;
#else
      __arm_2d_impl_alpha_blending((color_int *)src_buf, src_stride,
                                   (color_int *)dest_buf, dest_stride,
                                   &copy_size, opa);
#endif
    }
  }
  /*Masked*/
    ...
    if (opa > LV_OPA_MAX) {
      ...
      __arm_2d_impl_src_msk_copy(
          (color_int *)src_buf, src_stride, (uint8_t *)mask, mask_stride,
          &copy_size, (color_int *)dest_buf, dest_stride, &copy_size);
#endif
    }
    /*Handle opa and mask values too*/
      ...
      return false;
#else
      __arm_2d_impl_gray8_alpha_blending((uint8_t *)mask, mask_stride,
                                         (uint8_t *)mask, mask_stride,
                                         &copy_size, opa);
...
static void lv_draw_arm2d_img_decoded(struct _lv_draw_ctx_t *draw_ctx,
  ...
  bool transform =
      draw_dsc->angle != 0 || draw_dsc->zoom != LV_IMG_ZOOM_NONE ? true : false;
  ...
  if (!mask_any && !transform && cf == LV_IMG_CF_TRUE_COLOR &&
...
#if LV_COLOR_DEPTH == 16
  else if (!mask_any && !transform && cf == LV_IMG_CF_RGB565A8 &&
           draw_dsc->recolor_opa == LV_OPA_TRANSP) {
  ...
  /*In the other cases every pixel need to be checked one-by-one*/
    ...
    lv_draw_mask_res_t mask_res_def =
    ...
    if (!transform) {
      if (LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED == cf) {
        ...
        __RECOLOUR_WRAPPER(
            ...
            if (draw_dsc->recolor_opa > LV_OPA_MIN) {
              __ARM_2D_PIXEL_BLENDING_OPA(
                  ...
                  (color_int *)&(chrome_key.full), draw_dsc->recolor_opa);
            }

            __PREPARE_LL_ACCELERATION__();

            if (blend_dsc.opa >= LV_OPA_MAX) {
              __arm_2d_impl_cl_key_copy((color_int *)src_buf_tmp, src_stride,
                                        (color_int *)dest_buf, dest_stride,
                                        &copy_size, (color_int)chrome_key.full);
            } else {
              __arm_2d_impl_alpha_blending_colour_keying(
                  (color_int *)src_buf_tmp, src_stride, (color_int *)dest_buf,
                  dest_stride, &copy_size, blend_dsc.opa,
                  (color_int)chrome_key.full);
            } is_accelerated = true;)
        /* *INDENT-ON* */
      } else if ((LV_COLOR_DEPTH == 32) && !mask_any &&
        ...
        __RECOLOUR_WRAPPER(
            ...
            uint8_t *mask_temp_buf = NULL; if (blend_dsc.opa < LV_OPA_MAX) {
              mask_temp_buf =
                  lv_mem_buf_get(copy_size.iHeight * copy_size.iWidth);
              if (NULL == mask_temp_buf) {
                LV_LOG_WARN("Failed to allocate memory for alpha mask,"
                            " use normal route instead.");
                break;
              }
              ...
              __arm_2d_impl_gray8_colour_filling_channel_mask_opacity(
                  mask_temp_buf, src_stride,
                  (uint32_t *)((uintptr_t)src_buf_tmp +
                               LV_IMG_PX_SIZE_ALPHA_BYTE - 1),
                  src_stride, &copy_size, 0xFF, blend_dsc.opa);

              __arm_2d_impl_src_msk_copy((color_int *)src_buf_tmp, src_stride,
                                         mask_temp_buf, src_stride, &copy_size,
                                         (color_int *)dest_buf, dest_stride,
                                         &copy_size);
              ...
              __arm_2d_impl_src_chn_msk_copy(
                  ...
                  (uint32_t *)((uintptr_t)src_buf_tmp +
                               LV_IMG_PX_SIZE_ALPHA_BYTE - 1),
                  src_stride, &copy_size, (color_int *)dest_buf, dest_stride,
                  &copy_size);
            }
        ...
        __RECOLOUR_WRAPPER(
            ...
            if (blend_dsc.opa >= LV_OPA_MAX) {
              __arm_2d_impl_copy((color_int *)src_buf_tmp, src_stride,
                                 (color_int *)dest_buf, dest_stride,
                                 &copy_size);
            } else {
              __arm_2d_impl_alpha_blending((color_int *)src_buf_tmp, src_stride,
                                           (color_int *)dest_buf, dest_stride,
                                           &copy_size, blend_dsc.opa);
            } is_accelerated = true;)
        /* *INDENT-ON* */
      }
    } else if (!mask_any
    ...
    __ARM_2D_CFG_SUPPORT_COLOUR_CHANNEL_ACCESS__
                ||
                ((LV_IMG_CF_TRUE_COLOR_ALPHA == cf) && (LV_COLOR_DEPTH == 32))
#endif
                    )) {
      ...
      __RECOLOUR_WRAPPER(
          ...
          static arm_2d_tile_t target_tile; arm_2d_region_t clip_region;
          static arm_2d_region_t target_region;
                  ...
                  .tLocation =
                      {
                          ...
                          .iY =
                              draw_ctx->clip_area->y1 - draw_ctx->buf_area->y1,
                      },
                  .tSize =
                      {
          ...
          arm_2d_tile_generate_child(&target_tile_origin, &clip_region,
                                     &target_tile, false);
          ...
          static arm_2d_tile_t mask_tile; mask_tile = source_tile;
          ...
          if ((LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED == cf) ||
            ...
            arm_2d_tile_transform_with_opacity(
                &source_tile, &target_tile, &target_region, source_center,
                ARM_2D_ANGLE((draw_dsc->angle / 10.0f)),
                draw_dsc->zoom / 256.0f, (color_int)LV_COLOR_CHROMA_KEY.full,
                blend_dsc.opa);
    ...
    __ARM_2D_CFG_SUPPORT_COLOUR_CHANNEL_ACCESS__
            ...
            arm_2d_tile_transform_with_src_mask_and_opacity(
                &source_tile, &mask_tile, &target_tile, &target_region,
                source_center, ARM_2D_ANGLE((draw_dsc->angle / 10.0f)),
                draw_dsc->zoom / 256.0f, blend_dsc.opa);
    ...
    if (!is_accelerated)
      while (blend_area.y1 <= y_last) {
        ...
        if (transform) {
          lv_draw_transform(draw_ctx, &transform_area, src_buf, src_w, src_h,
                            src_w, draw_dsc, cf, rgb_buf, mask_buf);
        } else {
          convert_cb(&transform_area, src_buf, src_w, src_h, src_w, draw_dsc,
                     cf, rgb_buf, mask_buf);
        }
        ...
        if (draw_dsc->recolor_opa > LV_OPA_MIN) {
          ...
          /* apply re-colour */
          __arm_2d_impl_colour_filling_with_opacity(
              (color_int *)rgb_buf, buf_w, &copy_size,
              (color_int)draw_dsc->recolor.full, draw_dsc->recolor_opa);
        }
#if LV_DRAW_COMPLEX
        ...
        if (mask_any) {
          ...
          for (y = blend_area.y1; y <= blend_area.y2; y++) {
            ...
            mask_res_line =
                lv_draw_mask_apply(mask_buf_tmp, blend_area.x1, y, blend_w);
 ...
 *   STATIC FUNCTIONS
 **********************/
/* Separate the image channels to RGB and Alpha to match LV_COLOR_DEPTH
 * settings*/
static void convert_cb(const lv_area_t *dest_area, const void *src_buf,
                       lv_coord_t src_w, lv_coord_t src_h,
                       lv_coord_t src_stride, const lv_draw_img_dsc_t *draw_dsc,
  ...
  if (cf == LV_IMG_CF_TRUE_COLOR || cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED) {
    ...
    src_tmp8 += (src_stride * dest_area->y1 * sizeof(lv_color_t)) +
                dest_area->x1 * sizeof(lv_color_t);
    ...
    src_tmp8 += (src_stride * dest_area->y1 * LV_IMG_PX_SIZE_ALPHA_BYTE) +
                dest_area->x1 * LV_IMG_PX_SIZE_ALPHA_BYTE;

    lv_coord_t src_new_line_step_px =
        (src_stride - lv_area_get_width(dest_area));
    lv_coord_t src_new_line_step_byte =
        src_new_line_step_px * LV_IMG_PX_SIZE_ALPHA_BYTE;
