...
static void lv_draw_swm341_dma2d_blend_fill(lv_color_t *dest_buf,
                                            lv_coord_t dest_stride,
                                            const lv_area_t *fill_area,
                                            lv_color_t color);

static void lv_draw_swm341_dma2d_blend_map(lv_color_t *dest_buf,
                                           const lv_area_t *dest_area,
                                           lv_coord_t dest_stride,
                                           const lv_color_t *src_buf,
                                           lv_coord_t src_stride, lv_opa_t opa);

static void lv_draw_swm341_dma2d_img_decoded(lv_draw_ctx_t *draw,
...
void lv_draw_swm341_dma2d_init(void) {
  ...
  DMA2D->L[DMA2D_LAYER_OUT].PFCCR =
      (LV_DMA2D_COLOR_FORMAT << DMA2D_PFCCR_CFMT_Pos);
}

void lv_draw_swm341_dma2d_ctx_init(lv_disp_drv_t *drv,
  ...
  //    lv_draw_swm341_dma2d_img_decoded;
  dma2d_draw_ctx->base_draw.wait_for_finish = lv_gpu_swm341_dma2d_wait_cb;
}

void lv_draw_swm341_dma2d_ctx_deinit(lv_disp_drv_t *drv,
                                     lv_draw_ctx_t *draw_ctx) {
  LV_UNUSED(drv);
  LV_UNUSED(draw_ctx);
}

void lv_draw_swm341_dma2d_blend(lv_draw_ctx_t *draw_ctx,
  ...
  if (dsc->mask_buf == NULL && dsc->blend_mode == LV_BLEND_MODE_NORMAL &&
    ...
    if (src_buf) {
      ...
      lv_draw_swm341_dma2d_blend_map(dest_buf, &blend_area, dest_stride,
                                     src_buf, src_stride, dsc->opa);
      done = true;
    } else if (dsc->opa >= LV_OPA_MAX) {
      lv_area_move(&blend_area, -draw_ctx->buf_area->x1,
                   -draw_ctx->buf_area->y1);
      lv_draw_swm341_dma2d_blend_fill(dest_buf, dest_stride, &blend_area,
                                      dsc->color);
      done = true;
    }
  }
...
static void lv_draw_swm341_dma2d_img_decoded(lv_draw_ctx_t *draw_ctx,
                                             const lv_draw_img_dsc_t *dsc,
                                             const lv_area_t *coords,
                                             const uint8_t *map_p,
                                             lv_img_cf_t color_format) {
...
static void lv_draw_swm341_dma2d_blend_fill(lv_color_t *dest_buf,
  ...
  DMA2D->NLR = ((area_w - 1) << DMA2D_NLR_NPIXEL_Pos) |
               ((area_h - 1) << DMA2D_NLR_NLINE_Pos);
...
static void lv_draw_swm341_dma2d_blend_map(
  ...
  if (opa >= LV_OPA_MAX) {
    ...
    DMA2D->L[DMA2D_LAYER_FG].PFCCR =
        (LV_DMA2D_COLOR_FORMAT << DMA2D_PFCCR_CFMT_Pos);

    DMA2D->L[DMA2D_LAYER_OUT].MAR = (uint32_t)dest_buf;
    DMA2D->L[DMA2D_LAYER_OUT].OR = dest_stride - dest_w;

    DMA2D->NLR = ((dest_w - 1) << DMA2D_NLR_NPIXEL_Pos) |
                 ((dest_h - 1) << DMA2D_NLR_NLINE_Pos);
