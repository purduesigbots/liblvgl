...
void lv_draw_sdl_draw_letter(lv_draw_ctx_t *draw_ctx,
  ...
  bool has_composite = lv_draw_sdl_composite_begin(
      ctx, &letter_area, clip_area, NULL, dsc->blend_mode, &t_letter, &t_clip,
      &apply_area);

  lv_draw_sdl_transform_areas_offset(ctx, has_composite, &apply_area, &t_letter,
                                     &t_clip);
