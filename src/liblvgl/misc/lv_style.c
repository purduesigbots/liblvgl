...
static void lv_style_set_prop_internal(
    lv_style_t *style, lv_style_prop_t prop_and_meta, lv_style_value_t value,
    void (*value_adjustment_helper)(lv_style_prop_t, lv_style_value_t,
                                    uint16_t *, lv_style_value_t *));
static void lv_style_set_prop_helper(lv_style_prop_t prop,
                                     lv_style_value_t value,
                                     uint16_t *prop_storage,
                                     lv_style_value_t *value_storage);
static void lv_style_set_prop_meta_helper(lv_style_prop_t prop,
                                          lv_style_value_t value,
                                          uint16_t *prop_storage,
                                          lv_style_value_t *value_storage);
...
const uint8_t
    _lv_style_builtin_prop_flag_lookup_table[_LV_STYLE_NUM_BUILT_IN_PROPS] = {
        ...
        [LV_STYLE_TRANSFORM_ZOOM] =
            LV_STYLE_PROP_EXT_DRAW | LV_STYLE_PROP_LAYER_REFR,
        ...
        [LV_STYLE_TEXT_FONT] =
            LV_STYLE_PROP_INHERIT | LV_STYLE_PROP_LAYOUT_REFR,
...
lv_style_prop_t lv_style_register_prop(uint8_t flag) {
  ...
  size_t required_size =
      (last_custom_prop_id + 1 - _LV_STYLE_LAST_BUILT_IN_PROP);
  ...
  LV_GC_ROOT(_lv_style_custom_prop_flag_lookup_table)
  [last_custom_prop_id - _LV_STYLE_NUM_BUILT_IN_PROPS] = flag;
...
void lv_style_set_prop_meta(lv_style_t *style, lv_style_prop_t prop,
                            uint16_t meta) {
  lv_style_set_prop_internal(style, prop | meta, null_style_value,
                             lv_style_set_prop_meta_helper);
}

lv_style_res_t lv_style_get_prop(const lv_style_t *style, lv_style_prop_t prop,
                                 lv_style_value_t *value) {
...
uint8_t _lv_style_prop_lookup_flags(lv_style_prop_t prop) {
  ...
  if (prop == LV_STYLE_PROP_ANY)
    return LV_STYLE_PROP_ALL; /*Any prop can have any flags*/
  ...
  if (LV_GC_ROOT(_lv_style_custom_prop_flag_lookup_table) != NULL &&
      prop < _lv_style_custom_prop_flag_lookup_table_size)
...
static void lv_style_set_prop_internal(
  ...
  if (style->prop_cnt > 1) {
    ...
    value_adjustment_helper(prop_and_meta, value, &props[style->prop_cnt - 1],
                            &values[style->prop_cnt - 1]);
  } else if (style->prop_cnt == 1) {
    if (LV_STYLE_PROP_ID_MASK(style->prop1) == prop_id) {
      value_adjustment_helper(prop_and_meta, value, &style->prop1,
                              &style->v_p.value1);
      return;
    }
    size_t size =
