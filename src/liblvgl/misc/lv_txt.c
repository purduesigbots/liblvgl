 ...
 * @param max_width max width of the text (break the lines to fit this size).
 * Set COORD_MAX to avoid line breaks
 * @param flags settings for the text from 'txt_flag_type' enum
 * @param[out] word_w_ptr width (in pixels) of the parsed word. May be NULL.
 * @param cmd_state pointer to a txt_cmd_state_t variable which stores the
 * current state of command processing
...
static uint32_t lv_txt_iso8859_1_next(const char *txt, uint32_t *i) {
  if (i == NULL)
    return txt[0]; /*Get the next char*/
