...
void lv_obj_redraw(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj) {
  ...
  bool com_clip_res =
      _lv_area_intersect(&clip_coords_for_obj, clip_area_ori, &obj_coords_ext);
  /*If the object is visible on the current clip area OR has overflow visible
   *draw it. With overflow visible drawing should happen to apply the masks
   *which might affect children */
  bool should_draw =
      com_clip_res || lv_obj_has_flag(obj, LV_OBJ_FLAG_OVERFLOW_VISIBLE);
  if (should_draw) {
...
static void refr_area_part(lv_draw_ctx_t *draw_ctx) {
   ...
   * In full double buffered mode wait here while the buffers are swapped and a
   * buffer becomes available*/
  bool full_sized = draw_buf->size == (uint32_t)disp_refr->driver->hor_res *
                                          disp_refr->driver->ver_res;
  if ((draw_buf->buf1 && !draw_buf->buf2) ||
    ...
    if (disp_refr->driver->screen_transp) {
      if (disp_refr->driver->clear_cb) {
        disp_refr->driver->clear_cb(disp_refr->driver,
                                    disp_refr->driver->draw_buf->buf_act,
                                    disp_refr->driver->draw_buf->size);
      } else {
        lv_memset_00(disp_refr->driver->draw_buf->buf_act,
                     disp_refr->driver->draw_buf->size *
  ...
  if (top_act_scr == NULL && top_prev_scr == NULL) {
    lv_area_t a;
    lv_area_set(&a, 0, 0, lv_disp_get_hor_res(disp_refr) - 1,
                lv_disp_get_ver_res(disp_refr) - 1);
    if (draw_ctx->draw_bg) {
...
static lv_res_t layer_get_area(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj,
  ...
  if (layer_type == LV_LAYER_TYPE_TRANSFORM) {
    ...
    if (!_lv_area_intersect(&clip_coords_for_obj, draw_ctx->clip_area,
                            &tranf_coords)) {
      return LV_RES_INV;
    }
    ...
    if (!_lv_area_intersect(&inverse_clip_coords_for_obj,
                            &inverse_clip_coords_for_obj, &obj_coords_ext)) {
      return LV_RES_INV;
    }
...
static void layer_alpha_test(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx,
  ...
  bool has_alpha;
  /*If globally the layer has alpha maybe this smaller section has not (e.g. not
   *on a rounded corner) If turns out that this section has no alpha renderer
   *can choose faster algorithms*/
  if (flags & LV_DRAW_LAYER_FLAG_HAS_ALPHA) {
    /*Test for alpha by assuming there is no alpha. If it fails, fall back to
    ...
    if (has_alpha) {
      layer_ctx->area_act.y2 =
          layer_ctx->area_act.y1 + layer_ctx->max_row_with_alpha - 1;
    }
  } else {
    has_alpha = false;
  }
  ...
  lv_draw_layer_adjust(draw_ctx, layer_ctx,
                       has_alpha ? LV_DRAW_LAYER_FLAG_HAS_ALPHA
...
void refr_obj(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj) {
  ...
  if (layer_type == LV_LAYER_TYPE_NONE) {
    ...
    lv_draw_layer_ctx_t *layer_ctx =
        lv_draw_layer_create(draw_ctx, &layer_area_full, flags);
    if (layer_ctx == NULL) {
      LV_LOG_WARN("Couldn't create a new layer context");
      return;
    }
    lv_point_t pivot = {.x = lv_obj_get_style_transform_pivot_x(obj, 0),
    ...
    if (flags & LV_DRAW_LAYER_FLAG_CAN_SUBDIVIDE) {
      ...
      layer_ctx->area_act.y2 =
          layer_ctx->area_act.y1 + layer_ctx->max_row_with_no_alpha - 1;
      if (layer_ctx->area_act.y2 > layer_ctx->area_full.y2)
        layer_ctx->area_act.y2 = layer_ctx->area_full.y2;
    }
