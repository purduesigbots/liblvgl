...
static lv_style_res_t get_prop_core(const lv_obj_t *obj, lv_part_t part,
...
void lv_obj_refresh_style(lv_obj_t *obj, lv_style_selector_t selector,
  ...
  if ((part == LV_PART_ANY || part == LV_PART_MAIN) &&
      (prop == LV_STYLE_PROP_ANY || is_layout_refr)) {
  ...
  if (prop == LV_STYLE_PROP_ANY ||
      (is_inheritable && (is_ext_draw || is_layout_refr))) {
...
lv_style_value_t lv_obj_get_style_prop(const lv_obj_t *obj, lv_part_t part,
  ...
  if (found != LV_STYLE_RES_FOUND) {
    if (part == LV_PART_MAIN &&
      ...
      if (cls) {
        value_act.num =
            prop == LV_STYLE_WIDTH ? cls->width_def : cls->height_def;
      } else {
        value_act.num = 0;
      }
    } else {
...
void lv_obj_set_local_style_prop_meta(lv_obj_t *obj, lv_style_prop_t prop,
                                      uint16_t meta,
                                      lv_style_selector_t selector) {
  lv_style_t *style = get_local_style(obj, selector);
...
lv_style_value_t _lv_obj_style_apply_color_filter(const lv_obj_t *obj,
                                                  uint32_t part,
                                                  lv_style_value_t v) {
  if (obj == NULL)
    return v;
