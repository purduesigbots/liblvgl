...
void lv_indev_read_timer_cb(lv_timer_t *timer) {
  ...
  if (indev_act->proc.disabled || indev_act->driver->disp->prev_scr != NULL)
    return; /*Input disabled or screen animation active*/
...
static void indev_keypad_proc(lv_indev_t *i, lv_indev_data_t *data) {
  ...
  /*Pressing*/
  else if (!dis && data->state == LV_INDEV_STATE_PRESSED &&
  ...
  /*Release happened*/
  else if (!dis && data->state == LV_INDEV_STATE_RELEASED &&
           prev_state == LV_INDEV_STATE_PRESSED) {
...
static void indev_button_proc(lv_indev_t *i, lv_indev_data_t *data) {
  ...
  if (prev_state != data->state) {
    if (data->state == LV_INDEV_STATE_PRESSED) {
      LV_LOG_INFO("button %" LV_PRIu32 " is pressed (x:%d y:%d)", data->btn_id,
                  x, y);
    } else {
      LV_LOG_INFO("button %" LV_PRIu32 " is released (x:%d y:%d)", data->btn_id,
                  x, y);
    }
  }
...
static void indev_proc_press(_lv_indev_proc_t *proc) {
  ...
  lv_obj_transform_point(indev_obj_act, &proc->types.pointer.act_point, true,
                         true);

  /*If a new object was found reset some variables and send a pressed event
   * handler*/
