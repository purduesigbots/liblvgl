...
void *lv_msg_subsribe_obj(uint32_t msg_id, lv_obj_t *obj, void *user_data) {
  ...
  /*If not added yet, add a delete event cb which automatically unsubcribes the
   * object*/
...
uint32_t lv_msg_get_id(lv_msg_t *m) { return m->id; }

const void *lv_msg_get_payload(lv_msg_t *m) { return m->payload; }

void *lv_msg_get_user_data(lv_msg_t *m) { return m->user_data; }
...
static void obj_delete_event_cb(lv_event_t *e) {
  ...
  sub_dsc_t *s_next;
  while (s) {
    /*On unsubscribe the list changes s becomes invalid so get next item while
     * it's surely valid*/
