 ...
 *  STATIC PROTOTYPES
 **********************/
static const uint8_t *imgfont_get_glyph_bitmap(const lv_font_t *font,
                                               uint32_t unicode);
static bool imgfont_get_glyph_dsc(const lv_font_t *font,
 ...
 *   GLOBAL FUNCTIONS
 **********************/
lv_font_t *lv_imgfont_create(uint16_t height,
                             lv_get_imgfont_path_cb_t path_cb) {
  LV_ASSERT_MSG(
      LV_IMGFONT_PATH_MAX_LEN > sizeof(lv_img_dsc_t),
...
static bool imgfont_get_glyph_dsc(const lv_font_t *font,
  ...
  if (!dsc->path_cb(dsc->font, dsc->path, LV_IMGFONT_PATH_MAX_LEN, unicode,
                    unicode_next)) {
    return false;
  }
