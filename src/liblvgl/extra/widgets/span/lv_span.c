  ...
  while (cur_span) {
    ...
    /* Whether the current line is the end line and does overflow processing */
    {
      ...
      if (last_snippet->txt[last_snippet->bytes] == '\0') {
        next_line_h = 0;
        lv_span_t *next_span =
            _lv_ll_get_next(&spans->child_ll, last_snippet->span);
      ...
      if (txt_pos.y + max_line_h + next_line_h - line_space >
          coords.y2 + 1) { /* for overflow if is end line. */
        if (last_snippet->txt[last_snippet->bytes] != '\0') {
          last_snippet->bytes = strlen(last_snippet->txt);
          last_snippet->txt_w = lv_txt_get_width(
              last_snippet->txt, last_snippet->bytes, last_snippet->font,
    ...
    for (i = 0; i < item_cnt; i++) {
      ...
      pos.y = txt_pos.y + max_line_h - pinfo->line_h -
              (max_baseline - pinfo->font->base_line);
